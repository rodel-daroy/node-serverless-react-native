 ...
 *     tags:
 *       - Bank account
 *     summary: Save the bank account for cash out
 *     description: Save the bank account for cash out
 *     requestBody:
 *       content:
 *         application/json:
 *          schema:
...
module.exports.post = async (request) => {
  ...
  } else if (accountHolderType != "individual" && accountHolderType != "business") {
    hasError = true;
    errors['accountHolderType'] = 'accountHolderType can only be individual or business';
  }
  if (routingNumber === '') {
    hasError = true;
  ...
  if (userId > 0) {
    ...
    if (hasError) {
      ...
      await DbHelper.dbInsert('user_bank_account',
        {
          user_id: userId,
          country: country,
          currency: currency,
