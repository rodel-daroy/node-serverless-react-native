...
const decryptWebhook = (requestBody) => {
  ...
  const cipher = crypto.createDecipheriv('aes-256-cbc', process.env.PASSBASE_SECRET_API_KEY, iv);
  const decryptedResultBytes = Buffer.concat([cipher.update(encryptedResult.slice(16)), cipher.final(),]);
