 ...
 *     responses:
 *       200:
 *        description: Create the promotion admin information
 */
module.exports.post = async (request) => {
  ...
  if (!name && !start_date && !end_date && !explanation && !reward_amount) {
    hasError = true;
  ...
  if (!hasError) {
    try {
      await models.promotion_setting.create({ name, start_date, end_date, reward_amount, explanation });
      statusCode = 200;
      message = 'Promotion has sucessfully created';
    } catch (err) {
