...
module.exports.get = async (request) => {
  ...
  if (userId > 0) {
    ...
    let totalUserActions = await DbHelper.query('SELECT * FROM user_actions WHERE object_type IN (:arr)', { arr: ['REPORT_USER', 'REPORT_POST', 'REPORT_COMMENT'] });
    ...
    for (let action of pageUserActions) {
      let item = {
        id: action.id,
        created_at: ApiHelper.convertUnixTimeToTime(action.created_at),
